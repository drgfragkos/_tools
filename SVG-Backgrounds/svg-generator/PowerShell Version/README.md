# SVG Generator (PowerShell)

This project is a simple command-line tool that generates SVG files with a
customizable background color. You can specify RGB values directly or use
predefined color presets. It is a PowerShell port of the original Python
version and produces identical SVG output.

## Features

- Generate SVG files with a full black background or specified RGB colors.
- Set RGB values with -R, -G, -B (validated to the 0-255 range).
- Choose from predefined color presets with -Preset (alias: -c).
- List all available color presets with -ListPresets (alias: -l).
- Choose the output filename with -OutputFile (alias: -o).
- PowerShell parameter sets prevent invalid combinations (e.g. mixing a
  preset with RGB values, or supplying only some of -R/-G/-B).

## Requirements

- PowerShell 7+ (recommended, works on Windows, Linux, and macOS), or
  Windows PowerShell 5.1.
- No external modules or dependencies are required.

If script execution is blocked on Windows, allow local scripts for the
current user:
...
- `Main.ps1` - entry point (argument handling and file output)
- `SvgGenerator.ps1` - the New-SvgContent function
- `Presets.ps1` - the color preset table and Get-PresetList function

## Usage

Generate an SVG file with specific RGB values:
...
Use a color preset (alias -c matches the Python version's flag):

```powershell
./Main.ps1 -c PastelTeal
```

Choose the output filename (default: output.svg):
