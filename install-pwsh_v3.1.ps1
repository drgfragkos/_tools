...
if ($h) {
    ...
    Write-Host ""
    Write-Host "  Pre-flight check: if any pwsh.exe processes are running, the MSI"
    Write-Host "  cannot replace the in-use binaries and silently defers the file"
    Write-Host "  swap to the next reboot (install reports success but 'pwsh -v'"
    Write-Host "  still shows the old version). The script therefore lists running"
    Write-Host "  pwsh processes before upgrading and offers to close them (asks"
    Write-Host "  first in interactive mode; closes them automatically in -Silent"
    Write-Host "  mode - save any work in open pwsh windows before running -Silent)."
    Write-Host ""
...
if ($needsElevation -or $isCoreHost) {
    ...
    if ($isCoreHost) {
        ...
        Write-Host "[INFO] The upgrade continues in the new window. This pwsh window may be closed by the update's pre-flight step so the binaries can be replaced."
...
# Function: List running pwsh.exe processes. While any of these are alive,
# the MSI cannot replace the in-use binaries: it reports success but defers
# the actual file swap to the next reboot, so pwsh keeps answering with the
# old version ("upgrade succeeded but version didn't change").
function Get-RunningPwshProcesses {
    @(Get-Process -Name "pwsh" -ErrorAction SilentlyContinue)
}

# Function: Close the given pwsh processes - gracefully first
# (CloseMainWindow, so windowed shells get a normal close), then force-kill
# any stragglers by process id. Returns $true if none remain afterwards.
function Stop-RunningPwshProcesses {
    ...
    foreach ($p in $Processes) {
        try {
            $p.Refresh()
            if (-not $p.HasExited) {
                Write-Host "[INFO] Force-terminating pwsh (PID $($p.Id))..."
                Stop-Process -Id $p.Id -Force -ErrorAction Stop
            }
        } catch {
            Write-Warning "Could not terminate pwsh (PID $($p.Id)): $_"
        }
    }
    Start-Sleep -Seconds 1

    return ((Get-RunningPwshProcesses).Count -eq 0)
}

# Function: Pre-flight gate before the winget upgrade/install. Detects
# running pwsh processes, shows them, and (with consent, or automatically
# in -Silent mode) closes them so the MSI can swap the binaries in place.
# Returns $true if no pwsh processes remain (safe to upgrade cleanly),
# $false if some are still running (upgrade will complete only on reboot).
function Invoke-PwshPreflight {
    $running = Get-RunningPwshProcesses
    if ($running.Count -eq 0) { return $true }

    Write-Warning "The following pwsh.exe processes are running. The upgrade cannot fully complete while they hold the PowerShell 7 binaries open:"
    $running | ForEach-Object {
        $cmdLine = try {
            (Get-CimInstance Win32_Process -Filter "ProcessId=$($_.Id)" -ErrorAction Stop).CommandLine
        } catch { "" }
        Write-Host ("  PID {0,-7} started {1}  {2}" -f $_.Id, $_.StartTime, $cmdLine)
    }
    Write-Host "[INFO] These may be open terminal windows/tabs or editor-embedded terminals (e.g. VS Code)."

    $consent = $Silent
    if (-not $Silent) {
        $consent = (Read-Host "Close these pwsh processes now so the update can complete? Unsaved work in them will be lost. (y/n)") -match '^[Yy]$'
    } else {
        Write-Host "[INFO] -Silent mode: closing running pwsh processes automatically."
    }

    if ($consent) {
        if (Stop-RunningPwshProcesses -Processes $running) {
            Write-Host "[INFO] All pwsh processes closed. Proceeding with the update."
            return $true
        }
        Write-Warning "Some pwsh processes could not be closed. The installer will defer replacing in-use files until the next reboot."
        return $false
    }

    Write-Warning "Continuing with pwsh still running: the install will report success, but the version change takes effect only after all pwsh processes exit or the machine reboots."
    return $false
}
...
# report whether it now matches (or exceeds) the expected version. Pass
# -PwshWasRunning when the pre-flight left pwsh processes alive, so the
# warning explains the deferred file swap instead of sounding like a
# failed install.
function Confirm-PwshVersion {
    param (
        [string]$ExpectedVersion,
        [switch]$PwshWasRunning
    )
    ...
    } elseif ($PwshWasRunning) {
        Write-Warning "pwsh still reports version '$current': the installer could not replace the in-use binaries because pwsh.exe was running during the install. Close all pwsh windows (or reboot) and the update to '$ExpectedVersion' will complete."
    } else {
...
function Invoke-PwshUpgrade {
    ...
    # Pre-flight: close running pwsh processes (with consent) so the MSI
    # can swap the binaries in place instead of deferring to a reboot.
    $pwshClear = Invoke-PwshPreflight
    ...
    switch ($LASTEXITCODE) {
        0 {
            ...
            Confirm-PwshVersion -ExpectedVersion $TargetVersion -PwshWasRunning:(-not $pwshClear)
        }
